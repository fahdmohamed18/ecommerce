      ...
      <div className='p-5 '>
        ...
        <div className='allProducts'>
          ...
          {products.map(function(product: ProductCart , idx: number){
            return <div className='flex items-center justify-between py-3  border-b-[1px] border-green-700/35' key={idx}>
                    <div className='flex items-center gap-5'>
                          <div>
                          <Image width={200} height={200} src={product.product.imageCover} alt={product.product.title} />
                          </div>

                          <div>
                            <h2>{product.product.title}</h2>
                            <p className='my-3 text-green-600 '>Price : {product.price} EGP</p>
                            <Button onClick={() => removeItem(product.product._id)}>Remove</Button>
                          </div>
                    </div>

                    <div className='flex items-center gap-2'>
                      <Button onClick={() => updateCartItem(product.product._id, product.count + 1)}>+</Button>
                      <p>{product.count}</p>
                      <Button onClick={() => updateCartItem(product.product._id, product.count - 1)}>-</Button>
                    </div>
          </div>
